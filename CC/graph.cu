...
void undirected_graph::readECLgraph() {
    ...
    numEdges = edges.size();
    printf("Num vertices = %lld ,", numVert);
    printf("Num Edges = %lld ,", numEdges);
    // long long newEdges = 0;
    int count = 0;
    // for (long long u = 0; u < numVert && count < 20; u++) {
    //     for (long long idx = vertices[u]; idx < vertices[u+1] && count < 20; idx++) {
...
void undirected_graph::csr_to_coo() {
    ...
    long num_edges_for_gpu = (long)(GPU_share * numEdges);
    size_t bytes = num_edges_for_gpu * sizeof(uint64_t);
    long num_edges_for_cpu = (numEdges) - num_edges_for_gpu;

    int isone = (int)GPU_share;
    if(isone == 1){
        num_edges_for_gpu = numEdges;
        num_edges_for_cpu = 0;
        bytes = num_edges_for_gpu * sizeof(uint64_t);
    }
