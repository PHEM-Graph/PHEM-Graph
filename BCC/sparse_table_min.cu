...
__inline__ __device__ int log2_int_2(int x) {
    return __float2int_rz(log2f(x));
}


__global__ void computeblocks_min(int* d_na, int m , int* d_a, int n){
...
__global__ void preprocess_init_min(int* d_a, int* d_lookupmin, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        d_lookupmin[i] = d_a[i];
    }
...
__global__ void build_sparse_table_min(int* d_lookupmin, int k, int i, int threads) {
...
__global__ void query_sol_min(int* d_lookupmin, int* d_qleft, int* d_qright, int m, int q, int* d_ansmin, int* d_a , int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < q) {
        int a = d_qleft[i];
        int b = d_qright[i];
...
void solveQ_min(int m, int* d_na, int q, int* d_left, int* d_right, int* d_ansmin , int* d_a , int n) {
    ...
    CUDA_CHECK(cudaMalloc(&d_lookupmin, lookupmin_size));
